...
for img_name in test_images:
    ...
    for coord in pkm_coordinates:
        ...
        parking_spot = cv2.resize(four_point_transform(image, coord), size_edges)
        parking_spot = cv2.cvtColor(parking_spot, cv2.COLOR_BGR2GRAY)
        parking_spot = cv2.convertScaleAbs(parking_spot, alpha=1, beta=220)
        # parking_spot_sobel_x = cv2.convertScaleAbs(cv2.Sobel(parking_spot, cv2.CV_64F, 1, 0, ksize=5))
        # parking_spot_sobel_y = cv2.convertScaleAbs(cv2.Sobel(parking_spot, cv2.CV_64F, 0, 1, ksize=5))
        
        parking_spot_sobel = cv2.Sobel(parking_spot, cv2.CV_32F, dx=1, dy=1, ksize=5)
        edge_count = cv2.countNonZero(parking_spot_sobel)
        print(edge_count)
